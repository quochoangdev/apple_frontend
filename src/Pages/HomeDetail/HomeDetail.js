...
const HomeDetail = () => {
  ...
  const [imageDescribeColor, setImageDescribeColor] = useState("");
  ...
  useEffect(() => {
    (async () => {
      try {
        const axiosProduct = await axios.get(
  ...
  // set color default
  useEffect(() => {
    const randomColor = Math.floor(Math.random() * data?.imageDescribe.length);
    const firstAttributes = data?.imageDescribe.map((item) => Object.keys(item));
  ...
  }, [data?.imageDescribe, handleOptionClick]);
      ...
      <div className={cx("wrapper")}>
        <div className={cx("product")}>
          <div className={cx("product-left")}>
            {data?.imageDescribe.map((imgs, index) => {
              if (imgs.hasOwnProperty(imageDescribeColor)) {
                return (
                  <a
                    key={index}
                    className={cx("left-link")}
                    href={data && showImage ? showImage : data && imgs[imageDescribeColor][0]}
                  >
                    <img
                      className={cx("left-img")}
                      src={data && showImage ? showImage : data && imgs[imageDescribeColor][0]}
                      alt=""
                    />
                  </a>
                );
            ...
            <div className={cx("left-describe-image")}>
              <div className={cx("left-describe-block")} ref={describeScroll}>
                {data &&
                  data.imageDescribe.map((imgs) => {
                    if (imgs.hasOwnProperty(imageDescribeColor)) {
                      return imgs[imageDescribeColor].map((img, index) => {
                        return (
                          <div
